...
def extract_mbe(_y, _sr, _nfft, _nb_mel, _hop_len):
    S_amplitude = librosa.feature.melspectrogram(
        y=_y, sr=_sr, n_fft=_nfft, hop_length=_hop_len, n_mels=_nb_mel, power=1.0
    )
...
def load_desc_from_csv(csv_file, class_labels_dict):
    ...
    for index, row in df.iterrows():
        ...
        _desc_dict[name].append(
            [float(row[start_time_col]), float(row[end_time_col]), class_idx]
        )
...
def process_file(
    ...
    try:
        ...
        tmp_feat_file = os.path.join(
            feat_folder, "{}_{}.npz".format(audio_filename, "mon" if is_mono else "bin")
        )
...
def run_audio_processing():
    ...
    detection_data_path = Path("data") / "processed" / "detection"

    if not detection_data_path.exists():
        detection_data_path.mkdir(parents=True, exist_ok=False)
        root_folder = snapshot_download(
            repo_id=REPO_ID,
            repo_type="dataset",
            max_workers=4,
            local_dir=detection_data_path,
        )
        print(f"Dataset downloaded to: {root_folder}")
    else:
        print(f"Dataset already exists at: {root_folder}")
        root_folder = detection_data_path
    ...
    if X_train is not None and X_test is not None:
        ...
        print(
            "Error: Could not gather features. Check if .wav files exist and were processed."
        )
